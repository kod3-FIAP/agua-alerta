...
import type { ReceptorCreateInput, ReceptorSelect, ReceptorUpdateInput } from "../lib/types/types";
...
export const receptorService = {
  ...
  async getAllReceptores(args?: Parameters<typeof receptorRepository.findAll>[0]): Promise<ReceptorSelect[]> {
    return receptorRepository.findAll(args) as unknown as Promise<ReceptorSelect[]>;
  },

  async getReceptoresByZonaEmissaoId(idZonaEmissao: number): Promise<ReceptorSelect[]> {
    return receptorRepository.findByZonaEmissaoId(idZonaEmissao) as unknown as ReceptorSelect[];
  },
  ...
  async createReceptor(data: ReceptorCreateInput): Promise<ReceptorSelect> {
    ...
    return receptorRepository.create(validatedData as unknown as ReceptorCreateInput) as ReceptorSelect;
  },

  async updateReceptor(id: number, data: ReceptorUpdateInput): Promise<ReceptorSelect> {
