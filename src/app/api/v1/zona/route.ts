...
export const GET = withAuth(
  async (): Promise<
    NextResponse<ZonaEmissaoSelect[] | { message: string }>
  > => {
    try {
...
export const POST = withAuth(
  async (
    request,
  ): Promise<NextResponse<ZonaEmissaoSelect | { message: string }>> => {
    try {
      const body = (await request.json()) as ZonaEmissaoCreateInput;
